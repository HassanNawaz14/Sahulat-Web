...
CREATE POLICY "Users manage own budget expenses" ON public.budget_expenses
  FOR ALL USING (auth.uid() = user_id);

-- Updated-at triggers
CREATE TRIGGER set_updated_at_budget_categories BEFORE UPDATE ON public.budget_categories
  FOR EACH ROW EXECUTE FUNCTION public.set_updated_at();

CREATE TRIGGER set_updated_at_budget_expenses BEFORE UPDATE ON public.budget_expenses
  FOR EACH ROW EXECUTE FUNCTION public.set_updated_at();