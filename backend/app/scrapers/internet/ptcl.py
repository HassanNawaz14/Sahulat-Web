...
class PtclScraper(BaseScraper):
    ...
    consumer_number_pattern = r"^\d{8,11}$"

    BILL_URL = "https://dbill.ptcl.net.pk/"
    ...
    async def fetch_bill(self, consumer_number: str) -> ScrapedBill:
        if not self.validate_consumer_number(consumer_number):
            ...
            raise InvalidConsumerNumberError(
                f"Invalid PTCL account number: {consumer_number}"
            )
        ...
        async with get_client() as client:
            try:
                resp = await client.post(
                    self.BILL_URL,
                    data={"phone_no": consumer_number},
                )
                resp.raise_for_status()
            except Exception as e:
                raise PortalUnreachableError(
                    f"PTCL portal unreachable: {e}"
                ) from e
    ...
    def _parse_bill(self, html: str, consumer_number: str) -> ScrapedBill:
        ...
        bill = ScrapedBill()
        bill.raw_data = {"html_length": len(html), "phone_no": consumer_number}
        ...
        try:
            ...
            if not table:
                raise ParsingFailedError(
                    "Could not locate bill table in PTCL response"
                )
            ...
            raise ParsingFailedError(
                f"Failed to parse PTCL bill: {e}"
            ) from e

        if bill.amount_payable == 0 and bill.due_date is None:
            raise ParsingFailedError("Parsed PTCL bill data appears empty")
